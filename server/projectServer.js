...
module.exports = function (app) {
    app.route(urls.admin.projects)
        .get((request, response) => {
            const $1 = async function () {
                console.log('admin jobs api url GET = ', urls.admin.projects);
                ...
                if (user) {
                    if (user.role === Util.ROLE_ADMIN) {
                        ...
                        response.render("pages/admin/jobs/jobs", {
                            ...
                            script: urls.admin.projects,
                            // jobs: result
                        });
                    }
                } else {
                    response.redirect("/");
                    //response.status(404).send(`<!DOCTYPE html><pre>Cannot GET ${urls.admin.projects}</pre>`);
                }
            };
            $1();
        })
        .post((request, response) => {
            console.log('admin jobs api url POST = ', urls.admin.projects);
            const $1 = async function () {
        ...
        .delete((request, response) => {
            console.log('admin jobs api url DELETE = ', urls.admin.projects);
            const $1 = async function () {
        ...
        .put((request, response) => {
            console.log('admin jobs api url UPDATE = ', urls.admin.projects);
            const $1 = async function () {
                const data = request.body;
                const user = request.session.user;
                if (user) {
    ...
    app.route(Util.API_PREFIX + urls.admin.projects)
        .get((request, response) => {
            const $1 = async function () {
                const user = request.session.user;
                if (user) {
    ...
    app.route(Util.API_PREFIX + urls.admin.projects + "/extra/:id")
        .get((request, response) => {
            const $1 = async function () {
                const jobId = request.params.id;
                const user = request.session.user;
    ...
    app.route(Util.API_PREFIX + urls.admin.projects + "/dataTemplateByProject/:projectId")
        .get((request, response) => {
            const $1 = async function () {
    ...
    app.route("/ff" + urls.admin.projects)
        .post((request, response) => {
            response.sendFile(root + "/views/pages/admin/jobs/jobs.js");
        });

    app.route("/ss" + urls.admin.projects)
        .post((request, response) => {
